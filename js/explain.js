...
const ExplainEngine = (() => {
  ...
  const SAJU = {
    opening: [
      '오늘 {yearAnimal}띠의 기운이 강하게 작용하고 있어요.',
      '{element}의 기운이 당신의 운세를 이끌고 있습니다.',
      ...
      '{element}의 에너지가 번호 선택에 영향을 주고 있어요.',
    ...
    analysis: [
      '{dayGan}의 기운은 {element}에 속하며, 이는 특정 숫자 대역과 강한 친화력을 가져요.',
      '연주의 {yearAnimal}와 일주의 조합이 독특한 번호 패턴을 만들어냅니다.',
      '{element}의 오행은 안정과 조화를 상징하며, 균형 잡힌 번호가 유리해요.',
      '당신의 월주와 일주 사이의 상생 관계가 번호 선택의 핵심이에요.',
      '사주에서 {dayGan}{dayJi}의 조합은 특별한 수리적 의미를 담고 있어요.',
      '천간의 음양 배치가 홀짝 번호의 균형에 영향을 미쳤습니다.',
      '{element} 오행의 기운이 특정 번호 구간에 집중되어 있어요.',
      '당신의 연지와 일지 사이의 관계가 흥미로운 번호를 이끌어냈어요.',
      '사주 분석 결과, 이번 조합에서 {element}의 상생 에너지가 감지됩니다.',
      '네 기둥의 천간-지지 합이 독특한 수리 패턴을 형성하고 있어요.',
      '당신의 일간 {dayGan}이 가진 고유 에너지가 번호에 반영되었습니다.',
      '오행의 상생 순환이 이 번호들 사이에서 작동하고 있어요.',
      '{yearAnimal}띠의 행운 주기가 현재 상승세에 있습니다.',
      '월주와 시주의 상호작용이 번호 조합에 미묘한 영향을 주었어요.',
      '사주의 오행 균형이 이 번호 조합에서 잘 반영되고 있어요.'
    ],
    numbers: [
   ...
   * @param {Object} result - 생성 결과 객체
   * @returns {string} 완성된 설명 문자열
   */
  function generate(type, result) {
    ...
    const opening = fill(pick(tmpl.opening), ctx);
    const analysis = fill(pick(tmpl.analysis), ctx);
